...
- [x] **T-P1 Consolidate the seam into ONE packaged plugin** (the "1 plugin do conjunto"). `plugin/dream-machine-runtime/manifest.json` + `agent/lib/runtime-plugin.ts` public API; `pnpm pack:runtime` verifies files, contracts, motor tests, and writes `.pack/dream-machine-runtime.json`. `pnpm install:runtime` checks toolchain + ledgers. Ghost: macOS launchd / fleet daemon (→ T-T1). *Exit met:* 2026-06-27.
...
- [x] **T-R2 OAuth client registration crossing** (§ first external-effect additive client). `POST /oauth-crossing` + `agent/lib/oauth-crossing.ts`: shared `buildOAuthCrossingRequest` (dry-run/execute), Supabase POST on `execute:true`, Envelope `Shift`(kind `effect`)/`ShiftResult`(`effect_result`), `input_hash`=act `content_hash`, transport+custody; `client_secret` redacted. Governance: **keep L3** per `oauth-client.v1` contract. *Exit met:* `tests/oauth-crossing.test.ts` (2026-06-27).
