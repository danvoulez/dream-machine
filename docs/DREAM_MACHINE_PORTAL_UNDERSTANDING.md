...
helps the user navigate what entered, what became a candidate, what was admitted,
what is moving, what is waiting, and what needs human intervention.
...
- surface valid actions as buttons or approvals;
...
follow work as it becomes candidates, Acts, queue items, projections, ghosts,
doubts, receipts, and process runs.

## Dream Machine Model

The Dream Machine is a processual, ledger-backed runtime. Its law is:

> Everything may register. Only admitted Acts create consequence.
...
and governed by admission.

It contains the Acts that became truth for the system. An Act is the consequence
unit. Gate, admission, conformance, content hashes, receipts, and evidence live
around this side.

The portal must treat admitted Acts as authoritative and must not allow a UI row,
chat summary, tool result, or memory entry to outrank them.
...
trace table, and Act table all separately deciding process status.
...
world input
  ...
  -> sweeper or process proposes a candidate
  -> Gate admits or refuses
  -> admitted Act enters the LogLine ledger
  -> selectors observe admitted Acts
  -> queue, projections, and observability are rebuilt from authority
  -> executor and adapters produce evidence
  -> evidence is cited by new Acts or receipts
```

The connection points are content hashes, candidate references, receipt IDs,
evidence references, projection sources, queue source hashes, and process IDs.

In portal language:

- Envelope ledger: what happened around truth.
- LogLine ledger: what became truth.
...
into CAS plus a conformant registration Act through the Gate.
...
- Let the runtime decide whether an arrival is raw, candidate, ghost, doubted,
  admitted, queued, processing, blocked, or closed.
...
- Which admitted Acts caused this queue item?
...
- links back to Acts, candidates, queue items, traces, receipts, or evidence.
...
replacement for Gate, conformance, selectors, executor, or adapters.

It should:

- request projections when the user asks about runtime state;
- explain the distinction between raw registration, candidate, admitted Act,
  queue projection, ghost, doubt, and closed receipt;
...
- say when state is pending, stale, unadmitted, or merely observed;
...
- collapse envelope observations into admitted Acts;
- present raw ingress as an assertion by the system;
- use a dashboard row as authority when an Act or receipt is required.
...
- Boundary: never create consequence without admitted Acts and explicit authority
...
observations and admitted Acts, and exposes only valid, governed actions back to
the human.
