...
function extractMcpResults(message: McpJsonRpcMessage | null): { results: string[], error?: string } {
...
function pickListIssuesTool(tools: McpTool[]) {
  ...
  return tools.find(tool => preferred.includes(tool.name))
...
export async function fetchLinearIssuesViaMcp(token: string) {
  ...
  if (!init.ok || init.data?.error) {
    return {
      ok: false as const,
      error: init.data?.error?.message ?? `Linear MCP init failed: ${init.status} ${init.statusText}`,
    }
  }

  let sessionId = init.sessionId

  await linearMcpRequest(token, {
    jsonrpc: '2.0',
    method: 'notifications/initialized',
  }, sessionId)

  const toolsList = await linearMcpRequest(token, {
    jsonrpc: '2.0',
    id: 2,
    method: 'tools/list',
  }, sessionId)
  ...
  if (!toolsList.ok || toolsList.data?.error) {
    return {
      ok: false as const,
      error: toolsList.data?.error?.message ?? `Linear MCP tools/list failed: ${toolsList.status}`,
    }
  }
  ...
  if (!listTool) {
    ...
    return {
      ok: false as const,
      error: 'Linear MCP returned no tools',
    }
  }

  const call = await linearMcpRequest(token, {
    jsonrpc: '2.0',
    id: 3,
    method: 'tools/call',
    params: {
      name: listTool.name,
      arguments: buildListIssuesArgs(listTool),
    },
  }, sessionId)
  ...
  if (!call.ok || call.data?.error) {
    return {
      ok: false as const,
      error: call.data?.error?.message ?? `Linear MCP ${listTool.name} failed: ${call.status}`,
    }
  }
...
export async function fetchLinearIssuesViaGraphql(token: string) {
  for (const authorization of [`Bearer ${token}`, token]) {
    try {
      ...
      if (!issues.length) {
        return {
          ok: true as const,
          results: viewer ? [`Signed in as ${viewer} (no issues yet)`] : ['Connected (no issues yet)'],
        }
      }
  ...
  return {
    ok: false as const,
    error: 'Linear GraphQL API rejected the token',
  }
}
